				...
				try {
					...
					try {
						...
						if(!tokenized_file.valid()){
							auto error_message = 
								sl_string("[C&][ERROR][pre-processor] file: " + sl::to_str(file_name) + ": " + tokenized_file.error_message());
							return std::make_tuple(output, false, error_message.c_str());
						}
						else {
							auto preprocess_result = preprocess(tokenized_file.extract(), sl::to_str(file_name));
