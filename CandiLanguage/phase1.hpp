...
struct TokenTraits {
  ...
  static constexpr eToken::enum_type opening(
      const_char_vector::const_iterator it) {
    auto c = *it;
    if (c.is_numeric()) {
  ...
  // Given a token type curently open, returns true if this character would
  // close that token.
  static constexpr bool closing(const_char_vector::const_iterator it,
  ...
  // Given an open token type, source and start location: attempts to lex the
  // token. Returns the token that was lexed or an invalid token if the token
  // was not lexed.
  static Token lex(eToken::enum_type type, const const_char_vector& source,
                   const_char_vector::const_iterator it) {
  ...
  // Given a source code vector of const characters, tokenizes the source into a
  // vector of tokens.
  static token_vector tokenize(const TokenTraits::const_char_vector& source) {
    token_vector output;
    // While there are tokens left in the source,based on the top character of
    // the stack determine the next type of token we should attempt to lex.
    auto cursor = source.cbegin();
    if (!source.empty()) {
      while (cursor != source.end()) {
        auto opening_token_type = TokenTraits::opening(cursor);

        // If we have a valid token type, then we should lex until we reach the
        // end of the token.
        if (opening_token_type != eToken::invalid_) {
          auto result_token =
              TokenTraits::lex(opening_token_type, source, cursor);
  ...
  // Given a vector of tokens as a result of tokenize(), ensures there are no
  // invalid tokens at the end of the source. (Invalid at end of source
  // indicates an error in tokenization) Data from invalid back token may be
  // used to report errors.
  static bool validate(const token_vector& tokenized_source) {
    // If there is an invalid token at the end of the source, then the source is
    // invalid.
...
struct Token {
  ...
  Token(eToken::enum_type type,
        TokenTraits::const_char_vector::const_iterator begin,
        TokenTraits::const_char_vector::const_iterator end)
...
struct TokenizedSource {
  ...
  TokenizedSource(const TokenTraits::const_char_vector& source)
      : source(source),
        tokens(TokenTraits::tokenize(source)),
        is_valid(TokenTraits::validate(tokens)) {}
};
}  // namespace p1
